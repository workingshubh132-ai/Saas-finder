...
| `DEV_FIXTURE` is silently mistaken for real-world delivery, or a `REAL` request silently falls back to `DEV_FIXTURE` | An explicit runtime mode (`config.outboundMessageProviderMode`, `OUTBOUND_MESSAGE_PROVIDER_MODE` env var, default `DEV_FIXTURE`) decides which `OutboundMessageProvider` `createOutboundMessageProvider()` returns; `REAL` throws `ProviderNotConfiguredError` instead of ever returning `DevOutboundMessageProvider` — there is no fallback branch to fall back through | `tests/unit/outbound-message-provider-mode.test.ts` |

**`DEV_FIXTURE` is simulated transmission and must never be interpreted as real-world delivery. `REAL` mode requires an explicitly configured real provider and fails closed when unavailable.** This repository still has no real outbound provider implementation — `outbound-message-provider-factory.ts` has exactly one concrete class to construct (`DevOutboundMessageProvider`); `REAL` mode names a real destination this codebase does not yet build, not a working alternate path, and is documented that way in both `config.ts` and `docs/PHASE_A_CAPSTONE.md`.
